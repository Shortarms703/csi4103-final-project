...
def test_forward_single_variable_partial():
    x = Variable('x')
    f = Expression(x)
...
def test_back_single_variable_partial():
    x = Variable('x')
    f = Expression(x)
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert isinstance(partial_x.value, Constant)
    assert partial_x.value.value == 1


def test_forward_single_variable_partial2():
    x = Variable('x')
    y = Variable('y')
    f = Expression(x)
...
def test_back_single_variable_partial2():
    x = Variable('x')
    y = Variable('y')
    f = Expression(x)
    partials = back_propagation_partial(f)
    partial_y = partials.get_derivative(y)
    assert isinstance(partial_y.value, Constant)
    assert partial_y.value.value == 0
...
def test_forward_constant_partial(constant_expression):
    x = Variable('x')
    f = constant_expression
    partial_x = forward_propagation_partial(constant_expression, x)
    assert isinstance(partial_x.value, Constant)
    assert partial_x.value.value == 0


def test_back_constant_partial(constant_expression):
    x = Variable('x')
    f = constant_expression
    partials = back_propagation_partial(constant_expression)
    partial_x = partials.get_derivative(x)
    assert isinstance(partial_x.value, Constant)
    assert partial_x.value.value == 0
...
@pytest.fixture
def addition_expression():
    x = Variable('x')
    y = Variable('y')
    return Expression(Add(x, y))


def test_forward_addition_partial(addition_expression):
    x = Variable('x')
    y = Variable('y')
    f = addition_expression
    partial_x = forward_propagation_partial(f, x)
...
def test_back_addition_partial(addition_expression):
    x = Variable('x')
    y = Variable('y')
    f = addition_expression
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Constant(1))

    partial_y = partials.get_derivative(y)
    assert partial_y == Expression(Constant(1))

    partial_z = partials.get_derivative(Variable('z'))
    assert partial_z == Expression(Constant(0))
...
def test_back_multiplication_partial():
    x = Variable('x')
    y = Variable('y')
    f = Expression(Multiply(x, y))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Multiply(Constant(1), Variable('y')))
...
def test_back_partial_with_negatives():
    x = Variable('x')
    y = Variable('y')
    f = Expression(Add(x, Multiply(Constant(-1), y)))
    partials = back_propagation_partial(f)
    ...
    partial_y = partials.get_derivative(y)
    assert partial_y == Expression(Multiply(Constant(1), Constant(-1)))
...
def test_back_partial_with_negatives_and_coefficients():
    x = Variable('x')
    y = Variable('y')
    f = Expression(Add(Multiply(Constant(2), x), Multiply(Constant(-3), y)))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Multiply(Constant(1), Constant(2)))

    partial_y = partials.get_derivative(y)
    assert partial_y == Expression(Multiply(Constant(1), Constant(-3)))
...
def test_back_exponent_partials():
    x = Variable('x')
    f = Expression(Exponent(x, Constant(2)))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Multiply(Constant(1), Multiply(Constant(2), Exponent(Variable('x'), Constant(1)))))

    f = Expression(Exponent(x, Constant(3)))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Multiply(Constant(1), Multiply(Constant(3), Exponent(Variable('x'), Constant(2)))))

    f = Expression(Exponent(x, Constant(0)))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(Multiply(Constant(1), Multiply(Constant(0), Exponent(Variable('x'), Constant(-1)))))
...
def test_back_exponent_partials_addition_base():
    x = Variable('x')
    y = Variable('y')
    f = Expression(Exponent(Add(x, y), Constant(2)))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(
        Multiply(Constant(1), Multiply(Constant(2), Exponent(Add(Variable('x'), Variable('y')), Constant(1)))))
...
def test_back_sigmoid_partial():
    x = Variable('x')
    f = Expression(Sigmoid(x))
    partials = back_propagation_partial(f)
    partial_x = partials.get_derivative(x)
    assert partial_x == Expression(
            Multiply(Multiply(Constant(1), Sigmoid(x)), Add(Constant(1), Multiply(Constant(-1), Sigmoid(x)))))
...
def test_back_partial_compared_to_sympy_for_sigmoid():
    x = Variable('x')
    y = Variable('y')
    f = Expression(Sigmoid(Add(x, Multiply(Constant(-1), y))))
    partials = back_propagation_partial(f)
...
def test_partial_compared_to_sympy():
    ...
    back_partials = back_propagation_partial(s_loss)
    back_partial_W1_1 = back_partials.get_derivative(s_W1_1)
    back_partial_W1_2 = back_partials.get_derivative(s_W1_2)
    back_partial_b1 = back_partials.get_derivative(s_b1)
    ...
    # test ranges 0 to 10, -10 to 10, 2^0 to 2^20, -(2^0) to -(2^20)
    ranges = [-1, 0, 0.7, 1]  # , *[x ** 2 for x in range(0, 10)], *[-(x ** 2) for x in range(0, 10)]]
                ...
                for W1_2 in ranges:
                    for b1 in ranges:
                        for y in ranges:
                            ...
                            back_partial_W1_1_evaluated = back_partial_W1_1.evaluate({'X1': X1, 'X2': X2, 'W1_1': W1_1, 'W1_2': W1_2, 'b1': b1, 'y': y})
                            back_partial_W1_2_evaluated = back_partial_W1_2.evaluate({'X1': X1, 'X2': X2, 'W1_1': W1_1, 'W1_2': W1_2, 'b1': b1, 'y': y})
                            back_partial_b1_evaluated = back_partial_b1.evaluate({'X1': X1, 'X2': X2, 'W1_1': W1_1, 'W1_2': W1_2, 'b1': b1, 'y': y})
